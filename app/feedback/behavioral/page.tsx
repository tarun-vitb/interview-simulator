...
export default function BehavioralFeedbackPage() {
  ...
  useEffect(() => {
    ...
    if (data.rounds?.behavioral?.evaluation) {
      setEvaluation(data.rounds.behavioral.evaluation);
    } else {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 py-12 px-4">
      <div className="max-w-4xl mx-auto">
        <h1 className="text-3xl font-bold text-gray-900 mb-8">HR/Behavioral Interview Feedback</h1>
        ...
        <div className="bg-white rounded-lg shadow-lg p-8 mb-6">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <div className="text-center p-4 bg-blue-50 rounded-lg">
              ...
              <div className="text-gray-900 font-semibold">Communication</div>
            </div>
            <div className="text-center p-4 bg-green-50 rounded-lg">
              <div className="text-4xl font-bold text-green-600 mb-2">
                {scores.behavioral || 0}%
              </div>
              <div className="text-gray-900 font-semibold">Behavioral Response</div>
            </div>
            <div className="text-center p-4 bg-purple-50 rounded-lg">
              <div className="text-4xl font-bold text-purple-600 mb-2">
                {scores.confidence || 0}%
              </div>
              <div className="text-gray-900 font-semibold">Confidence</div>
            </div>
            <div className="text-center p-4 bg-orange-50 rounded-lg">
              <div className="text-4xl font-bold text-orange-600 mb-2">
                {scores.culturalFit || 0}%
              </div>
              <div className="text-gray-900 font-semibold">Cultural Fit</div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-lg p-8 mb-6">
          ...
          <div className="space-y-6">
            ...
            {feedback.behavioral && (
              <div>
                <h3 className="text-lg font-semibold text-gray-900 mb-3">Behavioral Response</h3>
                <p className="text-gray-900 text-base leading-relaxed">{feedback.behavioral}</p>
              </div>
            )}
            {feedback.confidence && (
              <div>
                <h3 className="text-lg font-semibold text-gray-900 mb-3">Confidence</h3>
                <p className="text-gray-900 text-base leading-relaxed">{feedback.confidence}</p>
              </div>
            )}
            {feedback.culturalFit && (
              <div>
                <h3 className="text-lg font-semibold text-gray-900 mb-3">Cultural Fit</h3>
                <p className="text-gray-900 text-base leading-relaxed">{feedback.culturalFit}</p>
              </div>
            )}
          </div>
        </div>
